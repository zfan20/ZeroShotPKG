...
from linkpred_utils import preprocess, SubgraphIterator, calc_mrr
from models import RGCNLinkPredict
from dgl.dataloading import GraphDataLoader


class HeteroGNNPretrainer:
    def __init__(self, graph, args):
        ...
        self.graph = graph
        self.num_edge_types = torch.unique(self.graph.edata['etype']).shape[0]
        self.num_nodes = self.graph.num_nodes()
        self.model = RGCNLinkPredict(self.num_nodes, self.num_edge_types, out_dim=self.args.hidden_size)
    ...
    def train(self):
        ...
        subg_iter = SubgraphIterator(self.graph, self.num_edge_types, 'uniform')
        ...
        for epoch, batch_data in enumerate(dataloader):
            ...
            #train_nids = train_nids.to(self.device)
            edges = edges.to(self.device)
            labels = labels.to(self.device)

            #embed = self.model(g, train_nids)
            embed = self.model(self.graph, nids=None)
            loss = self.model.get_loss(embed, edges, labels)
            self.optim.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0) # clip gradients
            self.optim.step()
            print('Epoch: ', epoch, ' has loss: ', loss.item())

    def save_item_embs(self, file_name):
        nids = np.arange(self.graph.num_nodes())
        self.model.eval()
        trained_item_embs = self.model(self.graph, nids).cpu().data.numpy().copy()

        with open(self.args.pkg_items_embs_path, 'wb') as f:
            np.save(trained_item_embs, f)
...
class Trainer:
    ...
    def get_full_sort_score(self, epoch, answers, pred_list):
        ...
        #return [recall[0], ndcg[0], recall[1], ndcg[1]], str(post_fix), [recall_dict_list, ndcg_dict_list, mrr_dict]
